...
function makeScope(fake: Record<PropertyKey, any>, p: PropertyKey) {
  const rawScope = scopables[p];
  fake[p] = new Proxy({},
      {
        get(target: any, key: string | symbol, _receiver: any): any {
          return target[key] ?? rawScope[key];
        },
        set(target: {}, key: string | symbol, value: any, _receiver: any): boolean {
          if (value === rawScope[key]) {
            delete target[p];
          } else {
            target[key] = value;
          }
 ...
 export default class ProxySandbox implements SandBox {
   ...
   constructor(name: string, globalContext = window) {
     ...
     this.proxy = <typeof this.proxy>proxy;
